...
from hour_feature_collect import hour_prepare_features
from strategy import strategy_acc, accumulated_return, econ_metrics, strategy_reg
...
def main():
    with tf.device('/GPU:0'):
     ...
     if len(arguments) == 0:
        ...
        #hourly
        # features = hour_prepare_features(arguments[0], arguments[1], arguments[2]) 
        print(features) 
        ...
        # ///////////// 
        # calculate the real target values in test dataset
        # when classificaion is used 
        # last_column_list = []
        # print(X_test.shape) 
        # for inner_list in X_test: 
        #     matrix = np.array(inner_list).reshape(int(arguments[4]), int(arguments[5]))   
        ...
        #         seen.add(item)
        # ret_Close = unique_elements[int(arguments[4]):] 
        # ret_Close = unique_elements
        # print(len(ret_Close))
        # print(len(y_pred))
        # #///////////////

         
        # # classificaion strategy
        ...
        append_to_file("start date and end date:" + arguments[1] + " to " + arguments[2]) 
        ...
        ## append the strategy results
        # append_to_file("prediction:"+ str(y_test))
        # append_to_file("prediction:"+ str(y_pred))
        # append_to_file("return:"+ str(ret_Close)) 
        # append_to_file("balance:"+ str(balance)) 
        ...
        # append_to_file("total Std_of_returns:" + str(econometrics[1]))
        # append_to_file("total sharpe_ratio:"+ str(econometrics[2]))
        # append_to_file("Return_per_annum:" + str(econometrics[3]))
        # append_to_file("Std_of_returns_per_annum:"+ str(econometrics[4])) 
        # append_to_file("annualized_sharp_ratio:" + str(econometrics[5]))
