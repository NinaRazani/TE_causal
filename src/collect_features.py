...
from collect_data import concat, extend_concat, fred_down, yfin_down, apply_talib_indicator, to_supervised_multi
...
def prepare_features(forex_id, start_date, end_date, flow=True):
     yah_df = yfin_down(forex_id, start_date, end_date,'1d', False) 
     ...
     for frx in major_fx:
         if frx != forex_id:
             currency_dfs[frx] = yfin_down(frx, start_date, end_date, '1d', False) 
     ...
     if flow==True:
         ...
         for frx in major_fx:
             if frx != forex_id:
                 cur_concat_dfs[frx] = concat( currency_dfs[frx], yah_df)  
        ...
        #      m.append(M) 
         ...
         for i in range(len(yah_df)):
             if i<=22:
                 ...
                 close_column = yah_df.iloc[i-22:i, 1].values 
                 S = np.std(close_column)  # type: ignore 
                 s.append(S)
                 M = np.mean(close_column) # type: ignore   
                 m.append(M) 
        ...
        # last_df['ret_Close'] = np.log(abs(yah_df['Close'])) - np.log(abs(yah_df['Close'].shift(1)))
         last_df['ret_Close'] = (yah_df['Close']-yah_df['Close'].shift(1)) / yah_df['Close'].shift(1)   #percentage return
         #last_df['ret_Close'] = np.log(yah_df['Close']/yah_df['Close'].shift(1))  #logarithmic return
