...
def to_supervised_multi(train_df, n_input, n_out, n_features):
    """_summary_
    ...
    for _ in range(len(train_df)):
        ...
        if out_end<len(train_df):
            x_input = train_df.iloc[in_start:in_end, 9:n_features].values.flatten()   #flatten is added because of encoder
            X.append(x_input)
            y.append(train_df.iloc[in_end:out_end, -1].values)
        in_start+=1
    return X, y
